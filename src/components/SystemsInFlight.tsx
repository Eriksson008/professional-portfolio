 ...
 * out of a CRM, into a search index, retrieved, reasoned over, returned.
 *
 * The *stages* come from `systems.ts`, which takes them from the project entry,
 * so no claim about the system is authored here. The section's own heading and
 * intro are hand-written framing, the same as every other section on the page.
 *
...
export function SystemsInFlight() {
  return (
    <Section id="systems" alt>
      ...
      {/* No reduced-motion guard on the variants: `Section` already sets
          `initial={false}` when motion is refused, which is what makes the
          cascade a no-op. Guarding here as well implied the default path was
          not safe, which it is. */}
      <m.ol className="flow" variants={gridStagger}>
        {clientAssistFlow.map((stage, i) => (
          <m.li className="flow-stage" key={stage.name} variants={headerItem}>
