...
export const HERO_FRAMES_NAME = 'astronaut-hero';

/**
 * Candidate B — the same film as a deterministic frame sequence.
 *
 ...
 *
 * What it costs is bytes and requests — 193 files instead of one — which is
 * exactly what the benchmark is there to weigh.
 */
export function AstronautHeroFrames() {
  const reduced = useReducedMotion();
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const lastIndex = useRef(-1);
  const decodeAround = useDecodeWindow();

  const frames = useHeroFrames(HERO_FRAMES_NAME, !reduced);
  ...
  const { runwayRef, heroRef, progress, settled } = useHeroRunway(scrub, draw, 'hero-frames');
  ...
  useEffect(() => {
    ...
    const fit = () => {
      ...
      if (canvas.width !== width || canvas.height !== height) {
        ...
        lastIndex.current = -1; // resizing clears the surface; force a repaint
      }
  ...
  // Paint the first frame the moment the sequence is ready, without waiting for
    ...
    >
      {scrub && (
        <canvas
          ...
          // The poster underneath carries the frame until every image has
          // arrived; a half-loaded scrub that skips frames is worse than a wait.
